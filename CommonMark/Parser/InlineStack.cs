...
namespace CommonMark.Parser
{
    ...
    internal sealed class InlineStack
    {
        ...
        public static void RemoveStackEntry(Block parent, InlineStack first, Subject subj, InlineStack last, CommonMarkSettings settings)
        {
            ...
            // potential scenario '*[*' ).
            if (curPriority > 0)
                PostProcessInlineStack(parent, null, first, last, curPriority, settings);
        }

        public static void PostProcessInlineStack(Block parent, Subject subj, InlineStack first, InlineStack last, InlineStackPriority ignorePriority, CommonMarkSettings settings)
        {
            while (ignorePriority > 0)
            {
                ...
                while (istack != null)
                {
                    ...
                    {
                        RemoveStackEntry(parent, istack, subj, istack, settings);
                    }
                        ...
                        if (iopener != null)
                        {
                            ...
                            if (retry)
                            {
                                ...
                                if (istack.Previous != null && iopener.Next != istack.Previous)
                                    RemoveStackEntry(parent, iopener.Next, subj, istack.Previous, settings);
                            ...
                            else
                            {
                                ...
                                RemoveStackEntry(parent, iopener, subj, istack, settings);
                            }
                        }
                        else if (!canClose)
                        {
